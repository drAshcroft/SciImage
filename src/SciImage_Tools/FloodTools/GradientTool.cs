...
namespace SciImage_Tools.FloodTools
{
        ...
        : Tool
    {
        ...
        public static ImageResource StaticImage
        {
            get
            {
                return SciImage.SciResources.SciResources.GetImageResource("Icons.glyphicons-98-gradient.png");
            }
        }
        ...
        private void RenderGradient(Surface surface, SciRegion clipRegion, CompositingMode compositingMode,
            PointF startPointF, ColorPixelBase startColor, PointF endPointF, ColorPixelBase endColor)
        {
            GradientRenderer gr = ToolEnvironment.GradientInfo.CreateGradientRenderer();
            ...
            for (int i = 0; i < Processor.LogicalCpuCount; ++i)
            {
                ...
                {
                    ThreadPool.Global.QueueUserWorkItem(wc, BoxedConstants.GetInt32(i));
                }
            }
        ...
        {
            ColorPixelBase startColor = ToolEnvironment.PrimaryColor;
            ColorPixelBase endColor = ToolEnvironment.SecondaryColor;

            if (this.shouldSwapColors)
            {
                if (ToolEnvironment.GradientInfo.AlphaOnly)
                {
            ...
            SciRegion clipRegion = DocumentWorkspace.Selection.CreateRegion();

            SaveRegion(clipRegion, clipRegion.GetBoundsInt(),surface.ColorPixelBase );

            RenderGradient(surface, clipRegion, ToolEnvironment.GetCompositingMode(), startPointF, startColor, endPointF, endColor);

            using (SciRegion simplified = Utility.SimplifyAndInflateRegion(clipRegion, Utility.DefaultSimplificationFactor, 0))
            {
            ...
            MeasurementUnit units = AppEnvironment.Environment.Units;
        ...
        private void CommitGradient()
        {
            ...
            using (SciRegion clipRegion = DocumentWorkspace.Selection.CreateRegion())
            {
        ...
        {
            this.toolCursor = new Cursor(SciImage.SciResources.SciResources.GetResourceStream("Cursors.GenericToolCursor.cur"));
            this.toolMouseDownCursor = new Cursor(SciImage.SciResources.SciResources.GetResourceStream("Cursors.GenericToolCursorMouseDown.cur"));
            this.Cursor = this.toolCursor;
            this.toolIcon = this.Image;

            this.startNub = new MoveNubRenderer(RendererList);
            this.startNub.Visible = false;
            this.startNub.Shape = MoveNubRenderer.MoveNubShape.Circle;
            RendererList.Add(this.startNub, false);

            this.endNub = new MoveNubRenderer(RendererList);
            this.endNub.Visible = false;
            this.endNub.Shape = MoveNubRenderer.MoveNubShape.Circle;
            ...
            ToolEnvironment.PrimaryColorChanged += new EventHandler(RenderBecauseOfEvent);
            ToolEnvironment.SecondaryColorChanged += new EventHandler(RenderBecauseOfEvent);
            ToolEnvironment.GradientInfoChanged += new EventHandler(RenderBecauseOfEvent);
            ToolEnvironment.AlphaBlendingChanged += new EventHandler(RenderBecauseOfEvent);
            AppEnvironment.Environment.UnitsChanged += new EventHandler(RenderBecauseOfEvent);
