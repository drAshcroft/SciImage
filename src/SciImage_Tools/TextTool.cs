...
namespace SciImage_Tools
{
        ...
        : Tool
    {
        ...
        {
            font = ToolEnvironment.FontInfo.CreateFont();
            if (mode != EditingMode.NotEditing)
            {
                this.sizes = null;
                RedrawText(true);
        ...
        {
            alignment = ToolEnvironment.TextAlignment;
            if (mode != EditingMode.NotEditing)
            {
                this.sizes = null;
                RedrawText(true);
        ...
        {
            SciBaseForm.RegisterFormHotKey(Keys.Back, OnBackspaceTyped);

            base.OnActivate();

            this.textToolCursor = new Cursor(SciImage.SciResources.SciResources.GetResourceStream("Cursors.TextToolCursor.cur"));
            ...
            ToolEnvironment.BrushInfoChanged += brushChangedDelegate;
            ToolEnvironment.FontInfoChanged += fontChangedDelegate;
            ToolEnvironment.FontSmoothingChanged += fontSmoothingChangedDelegate;
            ToolEnvironment.TextAlignmentChanged += alignmentChangedDelegate;
            ToolEnvironment.AntiAliasingChanged += antiAliasChangedDelegate;
            ToolEnvironment.PrimaryColorChanged += foreColorChangedDelegate;
            ToolEnvironment.SecondaryColorChanged += new EventHandler(BackColorChangedHandler);
            ToolEnvironment.AlphaBlendingChanged += new EventHandler(AlphaBlendingChangedHandler);
            
            this.threadPool = new ThreadPool();

            this.moveNub = new MoveNubRenderer(this.RendererList);
            this.moveNub.Shape = MoveNubRenderer.MoveNubShape.Compass;
        ...
        {
            SciBaseForm.UnregisterFormHotKey(Keys.Back, OnBackspaceTyped);
        ...
        private void UpdateStatusText()
        {
            ...
            {
                text = SciImage.SciResources.SciResources.GetString("TextTool.StatusText.StartTyping");
                image = null;
            }
        ...
        private Size StringSize(string s)
        {
            ...
            using (Surface window = ScratchSurface.CreateWindow(new Rectangle(0, 0, 1, 1)))
            {
                ...
                {
                    return Fonts.MeasureString(
                        ra2.Graphics, 
                        this.font, 
                        s, 
                        ToolEnvironment.AntiAliasing,
                        ToolEnvironment.FontSmoothing);
                }
            }
        }
        ...
        private void SaveHistoryMemento()
        {
            ...
            if (saved != null)
            {
                SciRegion hitTest = Selection.CreateRegion();
        ...
        private unsafe void DrawText(Surface dst, Font textFont, string text, Point pt, Size measuredSize, bool antiAliasing, Surface brush8x8)
        {
            ...
            using (RenderArgs renderArgs = new RenderArgs(this.ScratchSurface))
            {
                ...
                using (SciRegion clip = Selection.CreateRegion())
                {
                ...
                bool blending = ToolEnvironment.AlphaBlending;
                    ...
                    for (int y = pt.Y; y < pt.Y + measuredSize.Height; ++y)
                    {
                        ...
                        for (int x = pt.X + skipX; x < xEnd; ++x)
                        {
                            ...
                            else
                            {
                                ...
                                dst.SetPoint(x,y, NormalBlendOp.ApplyStatic(dstPixel, brushPixel));
                            }
        ...
        private void RedrawText(bool cursorOn)
        {
            ...
            using (SciRegion reg = Utility.RectanglesToRegion(Utility.InflateRectangles(rects, 3)))
            {
            ...
            if (cursorOn)
            {           
                using (Pen cursorPen = new Pen(Color.FromArgb(255, ToolEnvironment.PrimaryColor.ToColor()), 2))
                {
        ...
        private string GetStatusBarXYText()
        {
            ...
            Document.CoordinatesToStrings(AppEnvironment.Environment.Units, this.uls[0].X, this.uls[0].Y, out xString, out yString, out unitsAbbreviationXY);
        ...
        private void RenderText(object lineNumberObj)
        {
            ...
            using (Brush brush = ToolEnvironment.CreateBrush(false))
            {
                DrawText(ra.Surface, this.font, (string)this.lines[lineNumber], this.uls[lineNumber], this.sizes[lineNumber], ToolEnvironment.AntiAliasing, brush);
            }
        }
        ...
        public TextTool(DocumentWorkspace documentWorkspace)
            : base(documentWorkspace,
                   SciImage.SciResources.SciResources.GetImageResource("Icons.glyphicons-104-text.png"),
                   "Text Stamp",
