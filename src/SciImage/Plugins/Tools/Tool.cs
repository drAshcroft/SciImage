...
namespace SciImage.Plugins.Tools
{
          ...
          IHotKeyTarget
    {
        ...
        protected MouseButtons lastButton = MouseButtons.None;
        protected Surface scratchSurface;
        private SciRegion saveRegion;
#if DEBUG
        ...
        public void RestoreRegion(SciRegion region)
        {
        ...
        public void SaveRegion(SciRegion saveMeRegion, Rectangle saveMeBounds, ColorPixelBase RequiredFormat)
        {
        ...
        public void SetScratch(Surface scratchSurface)
        {
           

            this.scratchSurface = scratchSurface;
        ...
        private void Activate()
        {
            ...
            this.handCursor = new Cursor(SciResources.SciResources.GetResourceStream("Cursors.PanToolCursor.cur"));
            this.handCursorMouseDown = new Cursor(SciResources.SciResources.GetResourceStream("Cursors.PanToolCursorMouseDown.cur"));
            this.handCursorInvalid = new Cursor(SciResources.SciResources.GetResourceStream("Cursors.PanToolCursorInvalid.cur"));
            ...
            this.trackingNub.Shape = MoveNubRenderer.MoveNubShape.Compass;
        ...
        protected virtual void OnKeyPress(KeyPressEventArgs e)
        {
            if (!e.Handled && DocumentWorkspace.Focused)
            {
                ...
                {
                    ToolEnvironment.Environment.SwapUserColors();
                    e.Handled = true;
                }
                else if (e.KeyChar == swapPrimarySecondaryChoice)
                {
                    ToolEnvironment.Environment.ToggleWhichUserColor();
                    e.Handled = true;
                }
                else if (e.KeyChar == decPenSizeShortcut)
                {
                    ToolEnvironment.Environment.AddToPenSize(-1.0f);
                    e.Handled = true;
                }
                else if (e.KeyChar == decPenSizeBy5Shortcut && (ModifierKeys & Keys.Control) != 0)
                {
                    ToolEnvironment.Environment.AddToPenSize(-5.0f);
                    e.Handled = true;
                }
                ...
                {
                    ToolMenuInfo[] toolInfos = ToolFactory.Factory.ToolInfos.ToArray();
                    ...
                    for (int t = 0; t < toolInfos.Length; ++t)
                    {
                        ...
                        ToolMenuInfo localToolInfo = toolInfos[newTool];
                        ...
                        if (char.ToLower(localToolInfo.HotKey) == char.ToLower(e.KeyChar))
                        {
                            if (!this.IsMouseDown)
                            {
                                ToolEnvironment.Environment.CurrentToolChoice = (localToolInfo.ToolType);
                            }
                        ...
                        switch (e.KeyChar)
                        {
                            ...
                            case (char)27: // Escape
                                if (this.mouseDown == 0 && !Selection.IsEmpty)
                                {
                                    ...
                                    documentWorkspace.PerformAction("DeselectAction");
                                }
        ...
        public Form AssociatedForm
        {
            get
            {
                return FormsManager.BaseForm.FindForm();
            }
        }
        ...
        protected void SetStatus(ImageResource statusIcon, string statusText)
        {
            ...
            {
                statusIcon = SciResources.SciResources.GetImageResource("Icons.MenuHelpHelpTopicsIcon.png");
            }
        ...
        #region clonestamp helps
        //why are these not in clone stamp?
                    ...
                    ToolBarConfigItems toolBarConfigItems)
        {
            ...
            this.toolInfo = new ToolMenuInfo(name, helpText, toolBarImage, hotKey, skipIfActiveOnHotKey,order, toolBarConfigItems, this.GetType());
