...
namespace SciImage.Plugins.Effects
{
        ...
        : IDisposable
    {
        ...
        private SciRegion renderRegion;
        private Rectangle[][] tileRegions;
        private SciRegion[] tileSciRegions;
            ...
            private void RenderImpl()
            {
                ...
                try
                {
                    for (int tile = start; tile < max; tile += inc)
                    {
                        ...
                        SciRegion subSciRegion = ber.tileSciRegions[tile];
                        ber.OnRenderedTile(new RenderedTileEventArgs(subSciRegion, ber.tileCount, tile));
                    }
                }
        ...
        public void ThreadFunction()
        {
            ...
            try
            {
                ...
                if (tileCount == 1)
                {
                    ...
                    SciRegion subSciRegion = this.tileSciRegions[0];
                    OnRenderedTile(new RenderedTileEventArgs(subSciRegion, this.tileCount, 0));
                    threadPool.Drain();
                    OnFinishedRendering();
                }
                else
                {
                    if (tileCount > 0)
                    {
                        ...
                        SciRegion subPdnRegion = this.tileSciRegions[0];
        ...
        private Rectangle[][] SliceUpRegion(SciRegion region, int sliceCount, Rectangle layerBounds)
        {
                                        ...
                                        int workerThreads)
        {
            ...
            this.tileSciRegions = new SciRegion[this.tileRegions.Length];
            for (int i = 0; i < this.tileRegions.Length; ++i)
            {
                SciRegion pdnRegion = Utility.RectanglesToRegion(this.tileRegions[i]);
                this.tileSciRegions[i] = pdnRegion;
            }
            ...
            this.threadPool = new ThreadPool(this.workerThreads, false);
        }
