...
using SciImage.Core.Exceptions;
...
using SciImage.Core.VectorAndNubControls;
...
using SciImage.SystemLayer.Dialogs;
       ...
       // private AppWorkspace appWorkspace;
        ...
        public void UpdateStatusBarToToolHelpText(Tool tool)
        {
            ...
            else
            {
                ...
                SetStatus(contextText, SciResources.SciResources.GetImageResource("Icons.MenuHelpHelpTopicsIcon.png"));
            }
        }
        ...
        private void UpdateDocInfoInStatusBar()
        {
            if (Document == null)
            {
                AppEnvironment.Environment. ImageInfoStatusText = string.Empty;
            }
            else if (Document != null)
            {
                ...
                Document.CoordinatesToStrings(AppEnvironment.Environment.Units,
                      Document.Width,
                     Document.Height,
                      out widthString,
                      out heightString,
                ...
                string imageText = string.Format(
                    ...
                    SciStatusBar.ImageInfoStatusBarFormat,
                    widthString,
                    units,
                    heightString,
                    units);

                AppEnvironment.Environment.ImageInfoStatusText = imageText;
            }
        }
        ...
        private void UpdateCursorInfoInStatusBar(int cursorX, int cursorY)
        {
            ...
            if (DocumentManager.Manager.ActiveDocumentWorkspace == null ||
                DocumentManager.Manager.ActiveDocumentWorkspace.Document == null)
            {
               AppEnvironment.Environment. CursorInfoText = string.Empty;
            }
            else
            {
                string xString = "";
                ...
                Document.CoordinatesToStrings(AppEnvironment.Environment.Units, cursorX, cursorY, out xString, out yString, out units);

                string cursorText = string.Format(
                    CultureInfo.InvariantCulture,
                   SciStatusBar.CursorInfoStatusBarFormat,
                    xString,
                    units,
                    yString,
                    units);

                AppEnvironment.Environment.CursorInfoText = cursorText;
            }
        ...
        private void UpdateHistoryButtons()
        {
            ...
            {
                 MenuManager.MainMenu.SetEnableSubMenu("HistoryUndoAction", true);
            }
            else
            {
                 MenuManager.MainMenu.SetEnableSubMenu("HistoryUndoAction", false);
            }

            if (History.RedoStack.Count > 0)
            {
                 MenuManager.MainMenu.SetEnableSubMenu("HistoryRedoAction", true);
            }
            else
            {
                 MenuManager.MainMenu.SetEnableSubMenu("HistoryRedoAction", false);
            }
        }
        #endregion
        ...
        public bool PerformAction(PluginAction action)
        {
            bool nullTool = false;
            bool result = false;
            ...
            try
            {
                ...
                {
                    result = action.PerformAction( null, -1);
                }
            }
            finally
            {
        ...
        public void PerformAction(Type actionType, string newName, ImageResource icon)
        {
            using (new WaitCursorChanger(this))
            {
                ...
                PluginAction action = actionAsObject as PluginAction;
        ...
        public void ZoomToSelection()
        {
            ...
            {
                using (SciRegion region = Selection.CreateRegion())
                {
        ...
        {
            string nameFormat = SciResources.SciResources.GetString("LayerPropertyChanging.HistoryMementoNameFormat");
            string haName = string.Format(nameFormat, PropertyName);

            LayerPropertyHistoryMemento lpha = new LayerPropertyHistoryMemento(
                haName,
                SciResources.SciResources.GetImageResource("Icons.MenuLayersLayerPropertiesIcon.png"),
                this,
        ...
        protected override void OnDocumentMouseEnter(EventArgs e)
        {
            if (Tool != null)
            {
                Tool.PerformMouseEnter();
            }
            base.OnDocumentMouseEnter(e);
        }

        protected override void OnDocumentMouseLeave(EventArgs e)
        {
            if (Tool != null)
            {
                Tool.PerformMouseLeave();
            }
            base.OnMouseLeave(e);
        }

        protected override void OnDocumentMouseDown(MouseEventArgs e)
        {

        
            if (Tool != null)
            {
                Tool.PerformMouseDown(e);
            }
            base.OnDocumentMouseDown(e);
        }

        protected override void OnDocumentMouseUp(MouseEventArgs e)
        {
            if (Tool != null)
            {
                Tool.PerformMouseUp(e);
            }
            base.OnMouseUp(e);
        }

        //protected override void OnDocumentMouseClick(MouseEventArgs e)
        //{
        //    if (Tool != null)
        //    {
        //        Tool.PerformClick();
        ...
        protected override void OnDocumentMouseMove(MouseEventArgs e)
        {
            if (Tool != null)
            {
                Tool.PerformMouseMove(e);
            }
            base.OnMouseMove(e);
        }

        //protected override void OnDocumentKeyPress(KeyPressEventArgs e)
        //{
        //    if (Tool != null)
        //    {
        //        Tool.PerformKeyPress(e);
        //    }
        //    base.OnKeyPress(e);
        //}
        ...
        //    if (Tool != null)
        //    {
        //        Tool.PerformKeyDown(e);
        //    }
        //    base.OnKeyDown(e);
        //}

        //protected override void OnDocumentKeyUp(KeyEventArgs e)
        //{
        //    if (Tool != null)
        //    {
        //        Tool.PerformKeyUp(e);
        ...
        public void ToolEnvironment_ToolClicked(object sender, ToolClickedEventArgs e)
        {
            SetToolFromType(e.ToolType);
        }
        ...
        {
            return ToolFactory.Factory.CreateTool(toolType, this);
        }
        ...
        private void UpdateSelectionInfoInStatusBar()
        {
            ...
            else
            {
                ...
                using (SciRegion tempSelection = Selection.CreateRegionRaw())
                {
                ...
                SetStatus(newStatusText, SciResources.SciResources.GetImageResource("Icons.SelectionIcon.png"));
            }
        }
        ...
        public void AddToMruList()
        {
            using (new PushNullToolMode(this))
            {
                ...
                int edgeLength =  MenuManager.MainMenu.MostRecentFiles.IconSize;
                ...
                using (RenderArgs ra = new RenderArgs(thumb))
                {
                    ...
                    if (MenuManager.MainMenu.MostRecentFiles.Contains(fullFileName))
                    {
                         MenuManager.MainMenu.MostRecentFiles.Remove(fullFileName);
                    }

                     MenuManager.MainMenu.MostRecentFiles.Add(mrf);
            ...
            : IFileDialogUICallbacks
        {
            public FileOverwriteAction ShowOverwritePrompt(IWin32Window owner, string pathName)
            {
                ...
                string title = SciResources.SciResources.GetString("SaveAs.OverwriteConfirmation.Title");
                string textFormat = SciResources.SciResources.GetString("SaveAs.OverwriteConfirmation.Text.Format");
                string fileName;
            ...
            public bool ShowError(IWin32Window owner, string filePath, Exception ex)
            {
                ...
                {
                    string title = SciInfo.GetBareProductName();
                    string message = SciResources.SciResources.GetString("FileDialog.PathTooLongException.Message");
        ...
        private void UpdateExifTags(Document document)
        {
            ...
            PropertyItem pi = Exif.CreateAscii(ExifTagID.Software, SciInfo.GetProductName(false));
          ...
          : IFileTransferProgressEvents
        {
            ...
            public void BeginOperation(IWin32Window owner, EventHandler callWhenUIShown, ICancelable cancelSink)
            {
                ...
                this.progressDialog.Text = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.TransferProgress.Title");
                this.progressDialog.Icon = Utility.ImageToIcon(SciResources.SciResources.GetImageResource("Icons.MenuFileOpenIcon.png").Reference);
                this.progressDialog.ItemText = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.ItemText.Initializing");
            ...
            private WorkItemFailureAction ShowFileTransferFailedDialog(Exception ex)
            {
                ...
                string formTitle = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.ItemFailureDialog.Title");

                Image taskImage = SciResources.SciResources.GetImageResource("Icons.WarningIcon.png").Reference;

                string introTextFormat = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.ItemFailureDialog.IntroText.Format");
                string introText = string.Format(introTextFormat, ex.Message);

                TaskButton retryTB = new TaskButton(
                    SciResources.SciResources.GetImageResource("Icons.MenuImageRotate90CWIcon.png").Reference,
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.RetryTB.ActionText"),
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.RetryTB.ExplanationText"));

                TaskButton skipTB = new TaskButton(
                    SciResources.SciResources.GetImageResource("Icons.HistoryFastForwardIcon.png").Reference,
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.SkipTB.ActionText"),
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.SkipTB.ExplanationText"));

                TaskButton cancelTB = new TaskButton(
                    SciResources.SciResources.GetImageResource("Icons.CancelIcon.png").Reference,
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.CancelTB.ActionText"),
                    SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.CancelTB.ExplanationText"));
            ...
            private void UpdateUI()
            {
                ...
                if (this.cancelRequested)
                {
                    ...
                    this.progressDialog.ItemText = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.ItemText.Canceling");
                ...
                else
                {
                    ...
                    string progressFormat = SciResources.SciResources.GetString("DocumentWorkspace.ShowFileDialog.ProgressText.Format");
        ...
        public static DialogResult ChooseFiles(Control owner, out string[] fileNames, bool multiselect, string startingDir)
        {
            ...
            using (IFileOpenDialog ofd = CommonDialogs.CreateFileOpenDialog())
            {
                if (startingDir != null)
                {
                ...
                ofd.Filter = fileTypes.ToString(true, SciResources.SciResources.GetString("FileDialog.Types.AllImages"), false, true);
                ofd.FilterIndex = 0;
            ...
            Surface saveScratchSurface)
        {
            ...
            using (IFileSaveDialog sfd = CommonDialogs.CreateFileSaveDialog())
            {
                ...
                sfd.Title = SciResources.SciResources.GetString("SaveAsDialog.Title");
        ...
        {
            Icon formIcon = Utility.ImageToIcon(SciResources.SciResources.GetImageResource("Icons.MenuFileSaveIcon.png").Reference);
            string formTitle = SciResources.SciResources.GetString("WarnAboutFlattening.Title");

            string introText = SciResources.SciResources.GetString("WarnAboutFlattening.IntroText");
            Image taskImage = null;

            TaskButton flattenTB = new TaskButton(
                SciResources.SciResources.GetImageResource("Icons.MenuImageFlattenIcon.png").Reference,
                SciResources.SciResources.GetString("WarnAboutFlattening.FlattenTB.ActionText"),
                SciResources.SciResources.GetString("WarnAboutFlattening.FlattenTB.ExplanationText"));

            TaskButton cancelTB = new TaskButton(
                TaskButton.Cancel.Image,
                SciResources.SciResources.GetString("WarnAboutFlattening.CancelTB.ActionText"),
                SciResources.SciResources.GetString("WarnAboutFlattening.CancelTB.ExplanationText"));
            ...
            using (new PushNullToolMode(this))
            {
                ...
                if (Document.Layers.Count > 1 && !newFileType.SupportsLayers)
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.PerformAction("FlattenAction");
                        }
                        else
                        {
                            return false;
                ...
                {
                    Utility.ErrorBox(this, SciResources.SciResources.GetString("SaveImage.Error.UnauthorizedAccessException"));
                }

                catch (SecurityException)
                {
                    Utility.ErrorBox(this, SciResources.SciResources.GetString("SaveImage.Error.SecurityException"));
                }

                catch (DirectoryNotFoundException)
                {
                    Utility.ErrorBox(this, SciResources.SciResources.GetString("SaveImage.Error.DirectoryNotFoundException"));
                }

                catch (IOException)
                {
                    Utility.ErrorBox(this, SciResources.SciResources.GetString("SaveImage.Error.IOException"));
                }

                catch (OutOfMemoryException)
                {
                    Utility.ErrorBox(this, SciResources.SciResources.GetString("SaveImage.Error.OutOfMemoryException"));
                }
        ...
        */
        ...
        public static Document LoadDocument(Control owner, string fileName, out FileType fileTypeResult, ProgressEventHandler progressCallback)
        {
            ...
            try
            {
                ...
                if (ftIndex == -1)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.ImageTypeNotRecognized"));
                    return null;
                }
            ...
            using (new WaitCursorChanger(owner))
            {
                ...
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.UnauthorizedAccessException"));
                }

                catch (SecurityException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.SecurityException"));
                }

                catch (FileNotFoundException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.FileNotFoundException"));
                }

                catch (DirectoryNotFoundException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.DirectoryNotFoundException"));
                }

                catch (PathTooLongException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.PathTooLongException"));
                }

                catch (IOException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.IOException"));
                }

                catch (SerializationException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.SerializationException"));
                }

                catch (OutOfMemoryException)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.OutOfMemoryException"));
                }

                catch (Exception ex)
                {
                    Utility.ErrorBox(owner, SciResources.SciResources.GetString("LoadImage.Error.Exception") + "\n" + ex.Message);
                }
