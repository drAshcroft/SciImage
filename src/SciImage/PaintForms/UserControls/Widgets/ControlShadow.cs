...
namespace SciImage.PaintForms.UserControls.Widgets
{
        ...
        : Control
    {
        ...
        public ControlShadow()
        {
            ...
            this.roundedEdgeUL = SciResources.SciResources.GetImageResource("Images.RoundedEdgeUL.png").Reference;
            this.roundedEdgeUR = SciResources.SciResources.GetImageResource("Images.RoundedEdgeUR.png").Reference;
            this.roundedEdgeLL = SciResources.SciResources.GetImageResource("Images.RoundedEdgeLL.png").Reference;
            this.roundedEdgeLR = SciResources.SciResources.GetImageResource("Images.RoundedEdgeLR.png").Reference;

            if (!SciInfo.IsFinalBuild && !betaTagDone)
            {
                betaTagDone = true;

                string betaTagStringFormat = "Some strings are still a little wierd.  ";
                string appName = SciInfo.GetFullAppName();
                string expiredDateString = SciInfo.ExpirationDate.ToShortDateString();
        ...
        private void DrawShadow(Graphics g, Rectangle clipRect)
        {
            if (this.occludingControl != null)
            {
                ...
                using (SciRegion backRegion = new SciRegion(clipRect))
                {
