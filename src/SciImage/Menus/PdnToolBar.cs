...
{
    public class SciToolBar 
        : Control,
          IPaintBackground
    {
        //the menus are automatically populated from the action plugin system.  
        //these are convience methods as well as organization of the many menus
        ...
        public SciToolBar()
        {
            ...
            //try
            {
                ToolMenuInfo[] Tools = ToolFactory.Factory.ToolInfos.ToArray();
        ...
        protected override void OnLayout(LayoutEventArgs e)
        {
            ...
            this.toolStripPanel.Location = new Point(0,0);// new Point(0, this.mainMenu.Bottom);
...
//this.mainMenu.Padding = new Padding(0, this.mainMenu.Padding.Top, 0, this.mainMenu.Padding.Bottom);
          ...
          //  this.commonActionsStrip.Location = new Point(0, 0);
            this.viewConfigStrip.Location = new Point(0, 0);// new Point(this.commonActionsStrip.Right, this.commonActionsStrip.Top);
        ...
        private void ToolChooserStrip_ChooseDefaultsClicked(object sender, EventArgs e)
        {
            ...
            using (ChooseToolDefaultsDialog dialog = new ChooseToolDefaultsDialog())
            {
                ...
                dialog.Shown += shownDelegate;
                dialog.SetToolBarSettings(ToolEnvironment.Environment.CurrentToolChoice, ToolEnvironment.Environment);

                ToolEnvironment defaultAppEnv = ToolEnvironment.Environment;

                try
                {
                    dialog.LoadUIFromToolEnvironment(defaultAppEnv);
                }

                catch (Exception)
                {
                    defaultAppEnv = ToolEnvironment.Environment;
                    defaultAppEnv.SetToDefaults();
                    dialog.LoadUIFromToolEnvironment(defaultAppEnv);
                }

                dialog.ToolType = ToolFactory.Factory.DefaultToolType;

                DialogResult dr = dialog.ShowDialog(this);

                if (dr != DialogResult.Cancel)
                {
                    ToolEnvironment newDefaultAppEnv = dialog.CreateToolEnvironmentFromUI();
                    newDefaultAppEnv.SaveAsDefaultToolEnvironment();
                    ToolEnvironment.Environment.LoadFrom(newDefaultAppEnv);
                    ToolFactory.Factory.DefaultToolType = dialog.ToolType;
                    ToolEnvironment.Environment.CurrentToolChoice = dialog.ToolType;
                }
            }
